...
void solve()
{
    ...
    if(n<=v2&&n*p2<answer)answer=n*p2;
    if(n<=v1&&n*p1<answer)answer=n*p1;
    ...
    for(int i : friendships)
    {
        for(int j=0, len=sums.size();j<len;j++)
        {
            ...
            if(!possible[newSum])
            {
                ...
                if(newSum<=v1&&n-newSum<=v2)answer=min(answer, newSum*p1 + (n-newSum)*p2);
            }
        }
    }

    if(answer==LDBL_MAX)puts("Impossible");
    else printf("%.2f\n", answer);
}
