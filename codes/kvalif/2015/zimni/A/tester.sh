...
do
	...
	for i in `seq -w 1 10`;
	do
		timeout 2s ./${folder%%/}/a1.exe < $tests/a1.$i.in > a;
		command_code=$?;
		if [[ "$command_code" = "0" ]]
		then
			if [[ "`diff -w a $tests/a1.$i.sol`" = "" ]]
			then
				result=$(($result+10));
	...
	for i in `seq -w 1 10`;
	do
		timeout 2s ./${folder%%/}/a2.exe < $tests/a2.$i.in > a;
		command_code=$?;
		if [[ "$command_code" = "0" ]]
		then
			if [[ "`diff -w a $tests/a2.$i.sol`" = "" ]]
			then
				result=$(($result+10));
				text="$text  \x1b[32m✓\x1b[0m";
			else
